...
def run_json(command: list[str], timeout: int = 180) -> dict[str, Any]:
    ...
    return parse_json_robust(result.stdout)


def parse_json_robust(raw: str) -> Any:
    """Parse the first JSON value, tolerating stray stdout noise.

    System frameworks (e.g. macOS E5RT under a sandbox that denies its
    ~/Library/Caches) may print diagnostics to stdout after the JSON the
    child wrote. json.loads would reject the trailing data; raw_decode
    stops at the first complete value instead. Falls back to the text
    between the first '{' and the last '}' when noise precedes the JSON.
    """
    text = raw.lstrip()
    try:
