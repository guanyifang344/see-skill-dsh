...
def user_agents_path() -> Path:
    return Path.home() / ".dsh" / "AGENTS.md"
...
def print_trigger_hint() -> None:
    print("下一步：不要拖图或粘贴图片。DSH 里拖入的图片对文本模型不可见，发本地路径或 URL，例如：")
    print("  使用 see 查看 /path/to/screenshot.png")
    print("或显式输入 /see。如果模型说不支持视觉却没有运行 scripts/see.sh，执行：")
    print("  python3 scripts/onboard.py --install-agents")
    print("规则写入 ~/.dsh/AGENTS.md，新会话即生效。")
...
def config_status() -> int:
    ...
    print(f"DSH 用户指令（~/.dsh/AGENTS.md）：{agents_path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--skip-agents", action="store_true", help="不写入 ~/.dsh/AGENTS.md 拒绝覆盖规则。")
    parser.add_argument("--install-agents", action="store_true", help="只写入 DSH 用户指令，不改供应商配置。")
...
def maybe_install_agents(args: argparse.Namespace, interactive: bool) -> None:
    if args.skip_agents:
        return
    if interactive and not confirm("写入 DSH 用户指令（~/.dsh/AGENTS.md），避免模型因不支持视觉而拒绝看图？"):
